...
class MetaRepository implements IBaseRepository<Meta>, IFindRepository<Meta> {
    ...
    async findById(id: number): Promise<Meta | IRepoError | null> {
        const idColumn = 'id';
        const sql = `SELECT id, app, author, build_on, environment, app_version, db_version, docker_image, docker_version, jenkins_version, maintenance_mode, last_modified, created_on FROM ${this.table} WHERE ${idColumn} = $1;`;
        const value = [id];
    ...
    /**
     * @description Only to use if meta-entry can't be found with id == 1.
     * @returns {Meta[]} 
     */
    ...
    async findMaintenance(id: number): Promise<Maintenance | IRepoError | null> {
        const idColumn = 'id';
        const sql = `SELECT id, build_on, maintenance_mode, last_modified, created_on FROM ${this.table} WHERE ${idColumn} = $1;`;
        const value = [id];
    ...
    async updateMaintenance(id: number, data: Partial<Meta>): Promise<Maintenance | IRepoError | null> {
        const timeStamp = Utils.getTimestampWithOffsetInfo(new Date());
        const sql = `UPDATE ${this.table}
        SET maintenance_mode = $1, last_modified = $2
        WHERE id = $3
        RETURNING id, build_on, maintenance_mode, created_on, last_modified;
        `;
        const values = [data.maintenance_mode, timeStamp, id];
