...
describe('Unit-tests (service), priority: class CloudService', () => {
    ...
    describe('Service tests, priority: fn upload()', () => {

        describe('Testing valid fn calls', () => {

            test('Call PutObjectCommand + fn send(), params: valid <CloudUploadContext>', async () => {
        ...
        describe('Testing invalid fn calls', () => {
            ...
            test('Call PutObjectCommand + fn send(), throw exception on mocked api error', async () => {
                const mockParam_params: CloudUploadContext = {
                    bucket: secrets.CLOUD_BUCKET,
                    key: `tickets/${mockId.tickets.new[0]}/0_${mockId.tickets.new[0]}.webp`,
                    buffer: Buffer.from([0x52,0x49,0x46,0x46,0x24,0x00,0x00,0x00,0x57,0x45,0x42,0x50]),
    ...
    describe('Service tests, priority: fn delete()', () => {
        ...
        describe('Testing valid fn calls', () => {
            ...
            test('Call DeleteObjectsCommand + fn send(), params: valid <CloudDeleteContext>', async () => {
                const cloudService = new CloudService();
                const mockSendFn = jest.fn().mockResolvedValue(undefined);
                jest.spyOn(cloudService as any, 'getR2Client').mockReturnValue({send: mockSendFn});
        ...
        describe('Testing invalid fn calls', () => {
            ...
            test('Call DeleteObjectsCommand + fn send(), throw exception on mocked api error', async () => {
                const cloudService = new CloudService();
                const mockSendFn = jest.fn().mockResolvedValue(undefined);
                const mockError = new Error('test-error');
